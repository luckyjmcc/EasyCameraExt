...
public class DefaultCameraActions implements EasyCamera.CameraActions {
    ...
    public void takePicture(final EasyCamera.Callbacks callbacks) {
        ...
        camera.getRawCamera().takePicture(rawCallbackOrNull(callbacks.getShutterCallback()),
                ...
                rawCallbackOrNull(callbacks.getPostviewCallback()), new Camera.PictureCallback() {
            @Override
            public void onPictureTaken(byte[] data, Camera camera) {
                if (callbacks.getJpegCallback() != null) {
                    callbacks.getJpegCallback().onPictureTaken(data, DefaultCameraActions.this);
                    //TODO is this the right CameraActions?
                }
    ...
    private Camera.PictureCallback rawCallbackOrNull(final EasyCamera.PictureCallback callback) {
        if (callback != null) {
            return new Camera.PictureCallback() {
                @Override
                public void onPictureTaken(byte[] data, Camera camera) {
                    callback.onPictureTaken(data, DefaultCameraActions.this);
                    //TODO is this the right CameraActions?
                }
            };
        }
